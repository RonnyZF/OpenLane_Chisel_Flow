...
def extract_approx_error_normal_dist():  # CORREGIR EL CALCULO DE ERROR
    ...
    got_normal.append(got_array)
    expected_normal.append(expected_array)

    # print(NMED_array)


def extract_approx_error_triangular_dist():  # CORREGIR EL CALCULO DE ERROR
    ...
    mean = statistics.mean(tmp_a)
    NMED_array.append(mean)
    got_triangular.append(got_array)
    expected_triangular.append(expected_array)


def extract_approx_error_discrete_dist():  # CORREGIR EL CALCULO DE ERROR
    ...
    mean = statistics.mean(tmp_a)
    NMED_array.append(mean)
    got_discrete.append(got_array)
    expected_discrete.append(expected_array)
...
def database_output(apr_num):
    ...
    with open("database.csv", "w") as file:
        ...
        for n in range(len(area_array)):
            writer.writerow(
                [apr_num, sum_array[n], area_array[n], power_array[n], NMED_array[n], got_normal[n], expected_normal[n], got_triangular[n], expected_triangular[n], got_discrete[n], expected_discrete[n]])
...
def main():
    ...
    sum_type_array = ["A0", "A1", "A2", "A3", "A4", "A5", "A6", "A7",
                      "A8", "A9", "L0", "T0", "T1", "T2", "T3", "T4", "T5", "T6"]

    # for a in range(len(sum_type_array)):
    #   for b in range(len(sum_type_array)):
    sum = sum_type_array[0]+","+sum_type_array[0]+",F0,F0,F0"
    sum_array.append(sum)
    clean_openlane_runs()
    prepare_chissel(str(2), sum)
    run_chissel()
    extract_approx_error_normal_dist()
    extract_approx_error_triangular_dist()
    extract_approx_error_discrete_dist()
    prepare_openlane()
    ...
    sum = sum_type_array[0]+","+sum_type_array[1]+",F0,F0,F0"
    sum_array.append(sum)
    clean_openlane_runs()
    prepare_chissel(str(2), sum)
    run_chissel()
    ...
    sum = sum_type_array[0]+","+sum_type_array[2]+",F0,F0,F0"
    sum_array.append(sum)
    clean_openlane_runs()
    prepare_chissel(str(2), sum)
    run_chissel()
